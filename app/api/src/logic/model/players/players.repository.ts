...
function normalizeMatchStats(raw: Record<string, unknown>, row: {
	...
	return {
		T: row.tries ?? pickInt(raw, "tries") ?? 0,
		TS: pickInt(raw, "try_saves") ?? 0,
		G: pickInt(raw, "goals") ?? 0,
		FG: pickInt(raw, "field_goals") ?? 0,
		TA: row.tryAssists ?? pickInt(raw, "try_assists") ?? 0,
		LB: pickInt(raw, "line_breaks") ?? 0,
		LBA: pickInt(raw, "line_break_assists") ?? 0,
		TCK: row.tackles ?? pickInt(raw, "tackles") ?? 0,
		TB: pickInt(raw, "tackle_breaks") ?? 0,
		MT: row.missedTackles ?? pickInt(raw, "missed_tackles") ?? 0,
		OFH: pickInt(raw, "off_half", "ofh") ?? 0,
		OFG: pickInt(raw, "off_game", "ofg") ?? 0,
		ER: row.errors ?? pickInt(raw, "errors") ?? 0,
		FTF: pickInt(raw, "free_tries", "ftf") ?? 0,
		MG: row.metresGained ?? pickInt(raw, "metres_gained") ?? 0,
		KM: row.kickMetres ?? pickInt(raw, "kick_metres") ?? 0,
		KD: pickInt(raw, "kick_defusals") ?? 0,
		PC: pickInt(raw, "penalties_conceded") ?? 0,
		SB: pickInt(raw, "sin_bin") ?? 0,
		SO: pickInt(raw, "send_off") ?? 0,
		TOG: row.timeOnGround ?? pickInt(raw, "time_on_ground") ?? 0,
		FDO: pickInt(raw, "forced_drop_out") ?? 0,
		TO: pickInt(raw, "forced_turn_over") ?? 0,
		SAI: pickInt(raw, "sai") ?? 0,
		EFIG: pickInt(raw, "efig") ?? 0,
	};
}
